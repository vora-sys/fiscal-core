...
function rioPretoDaEvaIsswebOptions(array $argv): array
{
    $options = [
        'send' => false,
        'tomador_doc' => '12345678909',
        'tomador_nome' => 'TOMADOR DE TESTE RIO PRETO DA EVA',
        'numero' => date('His'),
    ];
...
function rioPretoDaEvaIsswebPayload(array $options): array
{
    return [
...
$options = rioPretoDaEvaIsswebOptions($argv);
if (($options['help'] ?? false) === true) {
    echo rioPretoDaEvaIsswebUsage(basename((string) $argv[0])).PHP_EOL;
    exit(0);
}

$projectRoot = dirname(__DIR__, 2);
$envOverrides = nfseMunicipalBuildEnvOverrides('rio-preto-da-eva', 'homologacao', $projectRoot, [
...
$payload = rioPretoDaEvaIsswebPayload($options);

if (($options['send'] ?? false) !== true) {
    $config = ProviderRegistry::getInstance()->getConfigForMunicipio('rio-preto-da-eva');
...
$nfse = $fiscal->nfse('rio-preto-da-eva');
